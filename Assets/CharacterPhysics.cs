...
public class CharacterPhysics : MonoBehaviour {
    ...
    Vector2 horTargetVelocity = new Vector2(0, 0);
    ...
    public void VerticalRaycast() {
        ...
        for(int i = 0; i < startRayPoint.Length; ++i) {
            ...
            Debug.DrawRay(new Vector2(point.position.x, point.position.y + skin * -dir), Vector3.up * dir * Mathf.Abs(velY), Color.red, Time.deltaTime);
            if (ray) {
                rayHit = true;
    ...
    public void HorizontalRaycast() {
        ...
        float skin = 0.015f;
        float yOffest = 0.0005f;
        float velX = velocity.x * Time.deltaTime;
        int dir = (velX > 0 || velX < 0) ? (int)Mathf.Sign(velX) : 0;

        if (dir == 0) {
            return;
       }

        Transform[] startRayPoint = (dir == 1) ? rightRayPoints : leftRayPoints ;
        ...
        if(movementState == E_MOVE_STATE.RIGHT || movementState == E_MOVE_STATE.LEFT) { 
            ...
            for(int i = 0; i < startRayPoint.Length; ++i) {
                Transform point = startRayPoint[i];            
                RaycastHit2D ray = Physics2D.Raycast(new Vector2(point.position.x + skin * dir, point.position.y + yOffest), Vector3.right * dir, Mathf.Abs(velX), m_platform);
                //Debug.DrawRay(new Vector2(point.position.x + skin, point.position.y + yOffest), Vector3.right * distance,Color.black,0.1f);
                if (ray) {
                    velocity.x = 0;
                    horTargetVelocity.x = 0;
                    movementState = E_MOVE_STATE.STILL;
